  ...
  stages {
    ...
    stage ('Build') {
     ...
     stage ('Deploy-To-Tomcat') {
            steps {
           sshagent(['tomcat']) {
                sh 'scp -o StrictHostKeyChecking=no target/*.jar ubuntu@65.2.4.199:/prod/apache-tomcat-9.0.65/webapps/CRUDlets.jar'
              }      
           }       
    }
    ...
    stage ('Port Scan') {
     ...
     stage ('DAST') {
       steps {
          sshagent(['zap']) {
            sh 'ssh -o  StrictHostKeyChecking=no ubuntu@3.111.34.165 "docker run -t owasp/zap2docker-stable zap-baseline.py -t https://65.2.4.199:8443/CRUDlets/" || true'
        }
      }
    }
